...
def enregistrer_pointage_arrivee(personnel_id, date_pointage, heure_arrivee=None, motif_retard=None, notes=None, est_absent=False):
    ...
    if heure_arrivee is None:
        heure_arrivee = get_current_time()
            ...
            for emp in employes:
                ...
                with st.expander(f"{emp['prenom']} {emp['nom']} - {emp['poste']}"):
                    ...
                    with col1:
                        ...
                        if pointage and pointage.get('heure_arrivee'):
                    ...
                    with col2:
                        if pointage and pointage.get('heure_depart'):
                            st.success(f"✅ Sortie: {pointage['heure_depart']} ({pointage['statut_depart']})")
                            if pointage.get('depart_avance_minutes', 0) > 0:
                                st.warning(f"⏰ Sortie anticipée: {pointage['depart_avance_minutes']} minutes")
                        else:
                            st.info("ℹ️ Sortie non enregistrée")
                    ...
                    with col_btn1:
                        if st.button("✅ Pointer l'entrée", key=f"arr_{emp_id}"):
                            ...
                            # CORRECTION : Utiliser des variables locales
                            try:
                                resultat_arrivee, retard = enregistrer_pointage_arrivee(
                    ...
                    with col_btn2:
                        if st.button("🚪 Pointer la sortie", key=f"dep_{emp_id}"):
                            ...
                            # CORRECTION : Utiliser des variables locales
                            try:
                                resultat_depart, avance = enregistrer_pointage_depart(
                                    emp_id, date.today(), heure_reelle, "", ""
                                )
                                if resultat_depart:
                                    st.success(f"✅ Sortie enregistrée à {heure_reelle.strftime('%H:%M:%S')}")
                                    time.sleep(0.5)
                                    st.rerun()
                                else:
                    ...
                    with col_btn3:
                        if st.button("❌ Marquer absent", key=f"abs_{emp_id}"):
                            # CORRECTION : Utiliser une variable locale
                            try:
                                resultat_absence = enregistrer_absence(
