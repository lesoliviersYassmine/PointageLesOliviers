...
def get_local_time():
    """
    Retourne l'heure locale en utilisant le décalage horaire configuré
    Par défaut: UTC+1 (Europe/Paris), ajustez le décalage selon votre besoin
    """
    # Heure UTC du serveur
    utc_now = datetime.utcnow()
    
    # Décalage horaire (en heures) - À AJUSTER SELON VOTRE FUSEAU HORAIRE
    # Exemples:
    # UTC+1 (France) : 1
    # UTC+2 (été France) : 2
    # UTC-5 (New York) : -5
    TIMEZONE_OFFSET = 1  # ← CHANGEZ CETTE VALEUR
...
def get_current_time():
    """Retourne l'heure actuelle (objet time)"""
    current_time, current_date = get_local_time()
    return current_time

def get_current_date():
    """Retourne la date actuelle (objet date)"""
    current_time, current_date = get_local_time()
    return current_date
